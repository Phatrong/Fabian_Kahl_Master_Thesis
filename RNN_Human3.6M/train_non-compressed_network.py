"""
    This file is the main to train non-compressed RNNs with GRU on the Human3.6M dataset
...
for i in range(1, runs+1):
    ...
    # creates the net
    net = MoCapRNN(n_joints*3, n_hidden)
    ...
    for e in range(epochs):
        ...
        # trains the net once
        loss, loss_normal, _, _, opt, net = train(net=net, data=data, mean=mean, std=std, 
        ...
        # tests the net once
        val_loss, val_loss_normal, _, _, val_loss_abs = test(net=net, data=val_data, mean=mean, std=std, 
    ...
    _, _, _, _, _ = test(net=best_net, data=val_data, mean=mean, std=std, 
                    ...
                    device=device, pred_samples=pred_samples, 
                    write_vids=True, run=i, compression='normal')
